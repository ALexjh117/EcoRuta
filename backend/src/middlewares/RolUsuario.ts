...
export const validateIdRolUsuarioYaExiste = async (req: Request, res: Response, next: NextFunction) => {
    await body('tipo_rol').custom(async (value)=>{
...
export const validateRolUsuarioBody = async (req: Request, res: Response, next: NextFunction) => {
    await body('tipo_rol').notEmpty().withMessage('El Tipo del rol no puede estar en blanco')
    .isIn(['Admin', 'Participante Rutas']).withMessage('El tipo de Rol debe ser Admin o Participante Rutas')
    .run(req);

    await body('id_usuario').notEmpty()
